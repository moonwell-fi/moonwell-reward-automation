...
import {
	ContractCall,
	createClients,
	baseClient as defaultBaseClient,
	moonbeamClient as defaultMoonbeamClient,
	optimismClient as defaultOptimismClient,
} from './utils';
...
async function getClosestBlockNumber(client: any, timestamp: number, blockTime: number): Promise<number> {
...
async function filterExcludedMarkets(markets: string[], chainId: number): Promise<string[]> {
	const excludedAddresses = excludedMarkets.filter((market) => market.chainId === chainId).map((market) => market.address);
...
export async function getMarketData(timestamp: number, env?: any, configOverrides?: ConfigOverrides) {
	...
	const moonbeamPrices = moonbeamPricesResponse.map((price, index) => {
		if (price.status === 'failure' || price.result === undefined) {
			throw new Error(
				`Failed to fetch price for Moonbeam market ${moonbeamNames[index]} (${moonbeamMarkets[index]}): ${price.error || 'RPC call failed'}`,
			);
		}
	...
	const glmrIndex = moonbeamMarkets.findIndex((market) => moonbeamNames[moonbeamMarkets.indexOf(market)] === 'GLMR');

	if (glmrIndex === -1) {
		throw new Error('GLMR market not found in Moonbeam markets');
	}

	const moonbeamNativePrice = formatUnits(moonbeamPrices[glmrIndex], 36 - 18);

	// Fetch prices from oracle for Base
	const basePricesResponse = await baseClient.multicall({
		contracts: baseMarkets.map(
			(market) =>
				({
					...baseOracleContract,
					functionName: 'getUnderlyingPrice',
	...
	const usdcIndex = baseMarkets.findIndex((market) => baseNames[baseMarkets.indexOf(market)] === 'USDC');
	...
	const opIndex = optimismMarkets.findIndex((market) => optimismNames[optimismMarkets.indexOf(market)] === 'OP');
	...
	const optimismNativePrice = formatUnits(opPrice, 36 - 18);

	const ethPrice =
		basePrices.find(
			(_price, index) =>
				baseMarkets[index].toLowerCase() === marketConfigs[8453].find((config) => config.nameOverride === 'ETH')?.address.toLowerCase(),
		) || BigInt(0);

	const cbBTCPrice =
		basePrices.find(
			(_price, index) =>
				baseMarkets[index].toLowerCase() === marketConfigs[8453].find((config) => config.nameOverride === 'cbBTC')?.address.toLowerCase(),
		) || BigInt(0);

	const eurcPrice =
		basePrices.find(
			(_price, index) =>
				baseMarkets[index].toLowerCase() === marketConfigs[8453].find((config) => config.nameOverride === 'EURC')?.address.toLowerCase(),
		) || BigInt(0);
	...
	const moonbeamSupplies = (
		await moonbeamClient.multicall({
	...
	const optimismSupplies = (
		await optimismClient.multicall({
			contracts: optimismMarkets.map(
				(market) =>
					({
						address: market as `0x${string}`,
	...
	const moonbeamWellSupplySpeeds = (
		await moonbeamClient.multicall({
			contracts: moonbeamMarkets.map(
				(market) =>
					({
	...
	const moonbeamWellBorrowSpeeds = (
		await moonbeamClient.multicall({
			contracts: moonbeamMarkets.map(
				(market) =>
					({
	...
	const optimismWellSupplySpeeds = (
		await optimismClient.multicall({
			contracts: optimismMarkets.map(
				(market) =>
					({
	...
	const optimismWellBorrowSpeeds = (
		await optimismClient.multicall({
			contracts: optimismMarkets.map(
				(market) =>
					({
	...
	const moonbeamNativeSupplySpeeds = (
		await moonbeamClient.multicall({
			contracts: moonbeamMarkets.map(
				(market) =>
					({
	...
	const optimismNativeSupplySpeeds = (
		await optimismClient.multicall({
			contracts: optimismMarkets.map(
				(market) =>
					({
	...
	const moonbeamNativeBorrowSpeeds = (
		await moonbeamClient.multicall({
			contracts: moonbeamMarkets.map(
				(market) =>
					({
	...
	const optimismNativeBorrowSpeeds = (
		await optimismClient.multicall({
			contracts: optimismMarkets.map(
				(market) =>
					({
	...
	const moonbeamWellSupplyPerDayUsd = moonbeamWellSupplyPerDay.map(
		(supplyPerDay) => Number(formatUnits(supplyPerDay, 18)) * Number(formatUnits(wellPrice, 36)),
	);

	const moonbeamWellBorrowPerDayUsd = moonbeamWellBorrowPerDay.map(
		(borrowPerDay) => Number(formatUnits(borrowPerDay, 18)) * Number(formatUnits(wellPrice, 36)),
	);
	...
	const moonbeamNativeSupplyPerDayUsd = moonbeamNativeSupplyPerDay.map(
		(supplyPerDay) => Number(formatUnits(supplyPerDay, 18)) * Number(moonbeamNativePrice),
	);

	const moonbeamNativeBorrowPerDayUsd = moonbeamNativeBorrowPerDay.map(
		(borrowPerDay) => Number(formatUnits(borrowPerDay, 18)) * Number(moonbeamNativePrice),
	);

	const baseNativeSupplyPerDayUsd = baseNativeSupplyPerDay.map(
		(supplyPerDay) => Number(formatUnits(supplyPerDay, 18)) * Number(baseNativePrice),
	);

	const baseNativeBorrowPerDayUsd = baseNativeBorrowPerDay.map(
		(borrowPerDay) => Number(formatUnits(borrowPerDay, 18)) * Number(baseNativePrice),
	);

	const optimismNativeSupplyPerDayUsd = optimismNativeSupplyPerDay.map(
		(supplyPerDay) => Number(formatUnits(supplyPerDay, 18)) * Number(optimismNativePrice),
	);

	const optimismNativeBorrowPerDayUsd = optimismNativeBorrowPerDay.map(
		(borrowPerDay) => Number(formatUnits(borrowPerDay, 18)) * Number(optimismNativePrice),
	);

	const moonbeamTotalSupplyUsd = moonbeamMarkets.map((_market, index) => {
		if (!moonbeamEnabled[index]) {
			// Only include markets that are enabled
			return 0;
		}
		...
		return (
			Number(formatUnits(supply, 8)) * Number(formatUnits(exchangeRate, 18 + digit - 8)) * Number(formatUnits(price, 36 - digit)) +
			boost -
			deboost
		);
	});
	...
	const moonbeamTotalBorrowsUsd = moonbeamMarkets.map((_market, index) => {
		...
		return Number(formatUnits(borrow, digit)) * Number(formatUnits(price, 36 - digit));
	});
	...
	function calculateNetworkTotalUSD(
		...
		markets.forEach((_market, index) => {
			...
			if (enabled) {
				...
				const borrowUSD = Number(formatUnits(borrow, digit)) * Number(formatUnits(price, 36 - digit));
	...
	const moonbeamTotalMarketPercentage = moonbeamNetworkTotalUsd / (moonbeamNetworkTotalUsd + baseNetworkTotalUsd + optimismNetworkTotalUsd);

	const baseTotalMarketPercentage = baseNetworkTotalUsd / (moonbeamNetworkTotalUsd + baseNetworkTotalUsd + optimismNetworkTotalUsd);

	const optimismTotalMarketPercentage = optimismNetworkTotalUsd / (moonbeamNetworkTotalUsd + baseNetworkTotalUsd + optimismNetworkTotalUsd);
	...
	const moonbeamNewWellSupplySpeeds = moonbeamMarkets.map((_market, index) => {
		...
		const totalWellPerEpochMarkets = config.totalWellPerEpoch * moonbeamTotalMarketPercentage * config.moonbeam.markets;
	...
	const baseNewWellSupplySpeeds = baseMarkets.map((_market, index) => {
		...
		const totalWellPerEpochMarkets = config.totalWellPerEpoch * baseTotalMarketPercentage * config.base.markets;
	...
	const optimismNewWellSupplySpeeds = optimismMarkets.map((_market, index) => {
		...
		const totalWellPerEpochMarkets = config.totalWellPerEpoch * optimismTotalMarketPercentage * config.optimism.markets;
	...
	const formatResults = (
		...
		markets.map((market: any, index: any) => ({
			...
			exchangeRate: Number(
				formatUnits(exchangeRates[index] as bigint, ((18 + digits[index]) as number) - 8), // 18 + market digits - mToken digits (8)
			),
			totalSupplyUnderlying:
				Number(formatUnits(supplies[index], 8)) * Number(formatUnits(exchangeRates[index] as bigint, ((18 + digits[index]) as number) - 8)),
			totalBorrowsUnderlying: Number(formatUnits(borrows[index] as bigint, digits[index] as number)),
			totalSupplyUSD: (() => {
				const value = Number(suppliesUsd[index].toFixed(2));
				if (value === 0 && enabled[index]) {
					console.log(
						`⚠️ ZERO SUPPLY USD ALERT: ${chainId === 1284 ? 'Moonbeam' : chainId === 8453 ? 'Base' : 'Optimism'} market ${names[index]} (${market}) has totalSupplyUSD = 0`,
					);
				}
				return value;
			})(),
			totalBorrowsUSD: (() => {
				...
				if (value === 0 && enabled[index]) {
					console.log(
						`⚠️ ZERO BORROW USD ALERT: ${chainId === 1284 ? 'Moonbeam' : chainId === 8453 ? 'Base' : 'Optimism'} market ${names[index]} (${market}) has totalBorrowsUSD = 0`,
					);
				}
				return value;
			})(),
			...
			nativeSupplyPerDay:
				chainId === 1284 // Moonbeam
					...
					: Number(formatUnits(nativeSupplyPerDay[index], digits[index] as number)).toFixed(18),
			nativeBorrowPerDay:
				chainId === 1284 // Moonbeam
					? Number(
							formatUnits(
					...
					: Number(formatUnits(nativeBorrowPerDay[index], digits[index] as number)).toFixed(18),
			...
			wellSupplyApr:
				suppliesUsd[index] > 0 ? Number(((wellSupplyPerDayUsd[index] / suppliesUsd[index]) * 365 * 100).toFixed(2)) : Number(0).toFixed(2),
			wellBorrowApr:
				borrowsUsd[index] > 0 ? Number(((wellBorrowPerDayUsd[index] / borrowsUsd[index]) * 365 * 100).toFixed(2)) : Number(0).toFixed(2),
			nativeSupplyApr:
				suppliesUsd[index] > 0
					? Number(((nativeSupplyPerDayUsd[index] / suppliesUsd[index]) * 365 * 100).toFixed(2))
					: Number(0).toFixed(2),
			nativeBorrowApr:
				borrowsUsd[index] > 0 ? Number(((nativeBorrowPerDayUsd[index] / borrowsUsd[index]) * 365 * 100).toFixed(2)) : Number(0).toFixed(2),
			percentage: percentages[index],
			minimumReserves:
				marketConfigs[chainId as keyof typeof marketConfigs]?.find((config) => config.address === market)?.minimumReserves ?? 0,
			reservesEnabled:
				marketConfigs[chainId as keyof typeof marketConfigs]?.find((config) => config.address === market)?.reservesEnabled ?? false,
			...
			newWellSupplyApr:
				suppliesUsd[index] > 0
					? Number((((newWellSupplySpeed[index] * 86400 * Number(wellPrice)) / suppliesUsd[index]) * 365 * 100).toFixed(2))
					: Number(0).toFixed(2),
			newWellBorrowApr:
				borrowsUsd[index] > 0
					? Number((((newWellBorrowSpeed[index] * 86400 * Number(wellPrice)) / borrowsUsd[index]) * 365 * 100).toFixed(2))
					: Number(0).toFixed(2),
			newNativeSupplyApr:
				suppliesUsd[index] > 0
					? Number((((newNativeSupplySpeed[index] * 86400 * Number(nativePrice)) / suppliesUsd[index]) * 365 * 100).toFixed(2))
					: Number(0).toFixed(2),
			newNativeBorrowApr:
				borrowsUsd[index] > 0
					? Number((((newNativeBorrowSpeed[index] * 86400 * Number(nativePrice)) / borrowsUsd[index]) * 365 * 100).toFixed(2))
					: Number(0).toFixed(2),
	...
	const [wethVaultTotalAssets, usdcVaultTotalAssets, eurcVaultTotalAssets, cbBTCVaultTotalAssets, meUSDCVaultTotalAssets] =
		await Promise.all([
