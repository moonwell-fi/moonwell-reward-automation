...
`;
		...
		const networkSummary = Object.values(marketData[networkId]).reduce(
			(prev, curr) => {
				return {
					...
					totalWellBySpeed:
						prev.totalWellBySpeed + curr.newWellSupplySpeed * marketData.totalSeconds + curr.newWellBorrowSpeed * marketData.totalSeconds,
					...
					totalNativeBySpeed:
						prev.totalNativeBySpeed +
						curr.newNativeSupplySpeed * marketData.totalSeconds +
						curr.newNativeBorrowSpeed * marketData.totalSeconds,
				};
			},
			{
				supplyUSD: 0,
				...
				supplyNative: 0,
				borrowNative: 0,
				totalNativeBySpeed: 0,
			},
		);

		const blockNumber =
			networkId === '10'
				? marketData.optimismBlockNumber
				: networkId === '1284'
					? marketData.moonbeamBlockNumber
					: networkId === '8453'
						? marketData.baseBlockNumber
						: null;
		...
		if (networkId === '1284') {
			...
			if (stkWellTotalSupply > 0) {
				...
				if (networkMarketData?.wellHolderBalance && Number(networkMarketData.wellHolderBalance) > 0) {
					...
					const targetAPY = 0.1; // 10% max APY cap
		...
		markdown += `| Total WELL to distribute Markets (Borrow + Supply) | ${Math.max(
			0,
			Number(networkSummary?.borrowWell) + Number(networkSummary?.supplyWell),
		)
			.toFixed(4)
			.toLocaleString()} WELL |\n`;
		...
		if (networkMarketData?.wellHolderBalance && Number(networkMarketData.wellHolderBalance) > 0) {
			...
			if (networkId === '8453') {
				...
				markdown += `| Total WELL to distribute (Sanity Check - Capped) | ${Math.max(
					0,
					Number(networkMarketData?.wellPerEpochDex) +
						Number(networkMarketData?.wellPerEpochSafetyModule) +
						Number(networkSummary?.totalWell) +
						cappedWellHolderBalance,
				)
					.toFixed(4)
					.toLocaleString()} WELL \n`;
			} else {
				markdown += `| Total WELL to distribute (Sanity Check) | ${Math.max(
					0,
					Number(networkMarketData?.wellPerEpochDex) +
						Number(networkMarketData?.wellPerEpochSafetyModule) +
						Number(networkSummary?.totalWell) +
		...
		if (Number(networkMarketData?.nativePerEpoch) !== 0) {
			...
			markdown += `| Total ${nativeToken} to distribute Markets (Borrow + Supply) | ${Math.max(
				0,
				Number(networkSummary?.borrowNative) + Number(networkSummary?.supplyNative),
			)
				.toFixed(4)
				.toLocaleString()} ${nativeToken} |\n`;
		...
		if (networkId === '8453') {
			...
			if (networkMarketData?.vaultAmounts) {
				...
				const totalVaultRewards =
					Number(vaultAmounts.USDC || 0) +
					Number(vaultAmounts.WETH || 0) +
					Number(vaultAmounts.EURC || 0) +
					Number(vaultAmounts.cbBTC || 0);
